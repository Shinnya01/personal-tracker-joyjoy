...
export const useDashboardStats = (source: () => TrackerItem[]) => {
  ...
  const reminderBuckets = computed<ReminderBuckets>(() => {
    ...
    const dated = source().filter((item) => item.deliveryReceiptDate);
    const today = dated.filter((item) => {
      const t = new Date(item.deliveryReceiptDate as string).getTime();
      return t >= now.getTime() && t <= endOfDay.getTime();
    });
    const overdue = dated.filter((item) => new Date(item.deliveryReceiptDate as string).getTime() < now.getTime());
